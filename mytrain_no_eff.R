mytrain_no_eff = function(num_vars)
{
    library(plyr)
    library(dplyr)
    library(caret)
...
#     model_rpart <- train(x = train_X_no_eff,
#                    y = train_y,
#                    method = 'rpart' ,
...
#     ##ada
#     set.seed(rbinom(n = 1,size = 10000,prob = 0.2))
#     model_ada <- train(x = train_X_no_eff,
#                    y = train_y,
#                    method = 'ada',
#                    trControl = fitControl,
#                    preProc = c("center", "scale"),
...
#     ##logistic penalized
#     set.seed(rbinom(n = 1,size = 10000,prob = 0.2))
#     model_plr <- train(x = train_X_no_eff,
#                        y = train_y,
#                        method = 'plr',
#                        trControl = fitControl,
#                        preProc = c("center", "scale"),
...
#                        metric = "ROC")
    ...
    ##nnet
#     set.seed(rbinom(n = 1,size = 10000,prob = 0.2))
#     model_nnet <- train(x = train_X_no_eff,
#                        y = train_y,
#                        method = 'nnet',
#                        trControl = fitControl,
#                        preProc = c("center", "scale"),
...
##bag
set.seed(rbinom(n = 1,size = 10000,prob = 0.2))
model_bag <- train(x = train_X_no_eff,
                   y = train_y,
                   method = 'bag',
                   trControl = fitControl,
                   preProc = c("center", "scale"),
...
#     do.call("rbind",results)
    list(models_no_eff = train_models, 
         test_X = test_X_no_eff,
         test_y = test_y)
